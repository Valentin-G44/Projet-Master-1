...
# Fonction pour rechercher les exploits disponibles pour un service spécifié et enregistrer les résultats dans un fichier
def search_exploits(service, output_file):
    command = ["msfconsole", "-q", "-x", "search {}; exit".format(service)]
    with open(output_file, "w") as file:
        subprocess.run(command, stdout=file)

# Fonction pour utiliser un exploit spécifié avec une cible IP et enregistrer les résultats dans un fichier
def execute_exploit(exploit_name, target_ip, output_file):
    command = ["msfconsole", "-q", "-x", "use {}; set RHOSTS {}; exploit; exit".format(exploit_name, target_ip)]
    with open(output_file, "w") as file:
        subprocess.run(command, stdout=file)
...
#4
            ...
            while True:
                ...
                if choix_exploit == "1":
                    service = input("Entrez le service cible : ")
                    output_file = input("Nom du fichier pour enregistrer les résultats de la recherche d'exploits : ")
                    search_exploits(service, output_file)


                # utilisation des exploit trouver
                ...
                elif choix_exploit == "4":
                    break  # Sortir du sous-menu et revenir au menu principal

                else:
                    print("Choix invalide. Veuillez saisir 1 à 4.")
