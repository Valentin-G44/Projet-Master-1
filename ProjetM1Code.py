...
#######################################################################################################################################################
#1/1/1/1/1/1/1/1/1 - Hydra usr/mdp inconnue 
# Fonction pour analyser la sécurité des mots de passe avec Hydra
def analyze_password_security(username_list, password_list, target, service, output_file):
    command = ["hydra", "-L", username_list, "-P", password_list, target, service]
    with open(output_file, "w") as file:
        subprocess.run(command, stdout=file)
    print("Analyse de la sécurité des mots de passe effectuée avec succès. Les résultats ont été enregistrés dans {}".format(output_file))

#2/2/2/2/2/2/2/2/2 - Hydra usr connue mdp inconnue
# Fonction pour tester une liste de mots de passe pour un identifiant connu avec Hydra
def test_password_list_for_username(username, password_list, target, service, output_file):
    command = ["hydra", "-l", username, "-P", password_list, target, service]
    with open(output_file, "w") as file:
        subprocess.run(command, stdout=file)
    print("Test de la liste de mots de passe pour l'identifiant {} effectué avec succès. Les résultats ont été enregistrés dans {}".format(username, output_file))

#3/3/3/3/3/3/3/3/3 - Hydra usr incconue mdp connue
# Fonction pour tester une liste d'identifiants pour un mot de passe connu avec Hydra
def test_username_list_for_password(username_list, password, target, service, output_file):
    command = ["hydra", "-L", username_list, "-p", password, target, service]
    with open(output_file, "w") as file:
        subprocess.run(command, stdout=file)
    print("Test de la liste d'identifiants pour le mot de passe {} effectué avec succès. Les résultats ont été enregistrés dans {}".format(password, output_file))

#4/4/4/4/4/4/4/4/4 - Hydra Connexion avec le usr/mdp
# Fonction pour tester l'authentification avec les identifiants récupérés auparavant avec Hydra
def test_authentication_with_credentials(credentials_file, target, service, output_file):
    command = ["hydra", "-C", credentials_file, target, service]
    with open(output_file, "w") as file:
        subprocess.run(command, stdout=file)
    print("Test d'authentification avec les identifiants récupérés effectué avec succès. Les résultats ont été enregistrés dans {}".format(output_file))
    ...
    while True:
        ...
        if choix_outil == "1":
            ...
            if choix_hydra == "1":
                ...
                username_list = input("Chemin vers la liste d'identifiants : ")
                password_list = input("Chemin vers la liste de mots de passe : ")
                target = input("Adresse IP de la cible : ")
                service = input("Service cible (par exemple, ssh, http, etc.) : ")
                output_file = input("Nom du fichier pour les résultats Hydra : ")
                analyze_password_security(username_list, password_list, target, service, output_file)
                ...
                service = input("Service cible (par exemple, ssh, http, etc.) : ")
                output_file = input("Nom du fichier pour les résultats Hydra : ")
                test_password_list_for_username(username, password_list, target, service, output_file)
                ...
                service = input("Service cible (par exemple, ssh, http, etc.) : ")
                output_file = input("Nom du fichier pour les résultats Hydra : ")
                test_username_list_for_password(username_list, password, target, service, output_file)
     
#Retirer ????????
            elif choix_hydra == "4":
                # Code pour l'option 4 : Test d'authentification avec les identifiants récupérés avant
                credentials_file = input("Chemin vers le fichier contenant les identifiants récupérés : ")
                target = input("Adresse IP de la cible : ")
                service = input("Service cible (par exemple, ssh, http, etc.) : ")
                output_file = input("Nom du fichier pour les résultats Hydra : ")
                test_authentication_with_credentials(credentials_file, target, service, output_file)
                                                                                                        #Retirer ????????   
